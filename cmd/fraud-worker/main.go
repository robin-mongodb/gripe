...
// fraud-worker: consumes SQS payment.created, mocks a fraud check, updates the payment.
...
func main() {
	...
	log.Info("fraud-worker up (idle — UC-13 pending)")
	<-await()
	log.Info("fraud-worker shutting down")
}
